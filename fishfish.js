...
//Función para cargar la carta con los datos de la API
const printCard = (pokemon) => {
  ...
  h_span.innerHTML = pokemon.height/10;
  w_span.innerHTML = pokemon.weight/10;
...
//Función para retornar los datos de un pokemon
...
const nameInButtons = async () => {
  for (let i=0; i<151; i++)
  {
    //Añadiendo className e id a cada uno
    let pokemon = await getData(i+1);
    let x=document.createElement("button");
    x.className="card";
    x.id=`${i+1}`;
    x.onclick=function() {fetchData(x.id)};
    var t=document.createTextNode(`${x.id}. ${capitalize(pokemon.name)}`);
    x.appendChild(t);
    cardsList.appendChild(x);

    //Dantole ciertas propiedades a cada carta
    let card = document.getElementById(`${i+1}`);
    card.style.cursor='pointer';
    card.style.color='#d9d9d9';
    card.style.fontWeight='bold';
    card.style.backgroundImage = 'linear-gradient(to bottom right, #353535, #3C6E71, #284b63)';
...
//Funciones decoradoras
    //Función para obtener los tipos de un pokemon en forma de array
const getTypes = (data) => {
  let arr=[];
  ...
  return arr;
    ...
    //Función para obtener las habilidades de un pokemon en forma de array
const getAbilities = (data) => {
  ...
  return arr;
    ...
    //Función para graficar las habilidades en la carta
function setAbilities (pokemon) {
  for (let i =0; i<pokemon.abilities.length; i++) {
      let ub = `a${i+1}`;
      let abl = document.getElementById(ub);
      
      abl.innerHTML = capitalize(pokemon.abilities[i]);
...
function setBGImage (p1, fondo) {
  if (p1.tipo[0] == 'plant'||p1.tipo[0] == 'grass') {
    ...
    //Función para capitalize un string
...
function readID ()
{
  let btn = document.getElementById('for-id');
  if (btn.value>=1&&btn.value<=151) {
    fetchData(btn.value);
  }
}
    //Read by Name
const readName = async () => {
  let intName = document.getElementById('for-name').value.toLowerCase();
  for (let i=0; i<151; i++) {
    const a = await getData(i+1);
    if (intName == a.name) {
      fetchData(i+1);
      break;
    }
...
//Botones;
document.getElementById("demo").onclick = function() {randomCard()};
document.getElementById('input-id').onclick = function() {readID()};
document.getElementById('input-name').onclick=function() {readName()};

/*
      --- IGNORE ---
*/
},{"cross-fetch":2}],2:[function(require,module,exports){
